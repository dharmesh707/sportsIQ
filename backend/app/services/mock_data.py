...
def build_mock_analysis(sport_type: SportType) -> AnalysisResult:
    ...
    return AnalysisResult(
        ...
        professional_comparison=(
            f"PREVIEW ONLY - {sport_type.value.replace('_', ' ').title()} analysis is "
            "not implemented yet. These numbers are placeholders so the interface "
            "can be demonstrated; they are not an analysis of your video."
        ),
        metrics={
        ...
        data_source="simulated",  # see app/services/sports_registry.py
